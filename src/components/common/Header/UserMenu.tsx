...
function Avatar({ src, nombre, inicial, size }: { src?: string; nombre: string; inicial: string; size: number }) {
  const cls = size === 40 ? 'w-10 h-10 text-sm' : 'w-16 h-16 text-2xl';
  if (src) {
    return <Image src={src} alt={nombre} width={size} height={size} className="w-full h-full object-cover" />;
  }
  return (
    <div className={`${cls} rounded-full bg-primary flex items-center justify-center font-bold text-primary-ink w-full h-full`}>
      {inicial}
    </div>
  );
}
...
export default function UserMenu({ isOpen, onToggle, onClose }: UserMenuProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {/* Botón avatar */}
      <button onClick={onToggle} className="group transition-transform hover:scale-95">
        <div className={`w-10 h-10 rounded-full overflow-hidden shrink-0 transition-shadow
          ...
          <Avatar src={avatar} nombre={nombreCompleto} inicial={inicial} size={40} />
        </div>
      </button>
        ...
        <div className="absolute right-0 top-full mt-2 w-80 z-50">
          <div className="bg-surface rounded-2xl border border-line p-4 shadow-lg">
            ...
            <div className="relative pb-8">
              ...
              <div className="absolute bottom-0 left-3">
                <div className="w-16 h-16 rounded-full overflow-hidden border-4 border-surface">
                  <Avatar src={avatar} nombre={nombreCompleto} inicial={inicial} size={64} />
                </div>
              </div>
            </div>
            ...
            <div className="px-1 pb-3 border-b border-line-strong">
              ...
              {usuario && <p className="text-sm text-ink-muted mt-0.5">ID: {usuario.id_usuario}</p>}
            </div>
